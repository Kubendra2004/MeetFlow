...
def send_whatsapp(message: str):
    """Send any WhatsApp message with automatic chunking for long content."""
    client = _get_client()
    if not client:
        return

    # Split into chunks of 1500 chars (Twilio limit is ~1600)
    ...
    for chunk in chunks:
        try:
            ...
            print(f"[WhatsApp] ✅ Sent chunk (SID: {msg.sid})")
        except Exception as e:
            print(f"[WhatsApp] ❌ Failed to send chunk: {e}")
